...
export class RestaurantQueries {
  ...
  async findByFilters(filters: SearchRestaurantQueryObject): Promise<Restaurant[]> {
    ...
    if (city) {
      qb.innerJoin('restaurant.address', 'address')
        .andWhere('LOWER(address.city) = LOWER(:city)', {
